...
/*
void drawPixelAnalysis (Mat mserImg)
{
    Mat anImg = Mat::zeros(mserImg.rows,mserImg.cols,CV_8UC1);   
    threshold(mserImg, mserImg, 100, 255, THRESH_BINARY);
    cout << "=======>> " << mserImg.cols << "  " << mserImg.rows << endl;
    for (int i = 0; i < mserImg.cols; i++)
    {
        int black = 0;
        int white = 0;
        for (int j = 0; j < mserImg.rows; j++)
        {
            cout << i << " " << j << "      " << mserImg.at<int>(i,j) << endl;
//            if ((int)mserImg.at<char>(i,j) == 255)
//                cout << "white" << endl;
//            else
//                cout << "black" << endl;
        }
...
int main (int argc, char ** argv)
{
    ...
    while(getline(inList, imgPath))
    {
        ...
        for (int i = 0; i < mserVec.size(); i++)
        {
            ...
            showMSERimg(MSERImage, mserVec[i], boundRect);
